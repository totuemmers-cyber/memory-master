...
export function StoryRecall({ difficulty, onComplete, onQuit }: ChallengeProps) {
  ...
  // Pause timer when tab hidden during memorize phase
  useEffect(() => {
  ...
  const handleAnswer = useCallback((answer: string) => {
    ...
    const q = story.questions[currentQIndex];
    ...
    const aq: AnsweredQ = { question: q, playerAnswer: answer, isCorrect, timeTakenMs: timeTaken };
  ...
  }, [story, currentQIndex, answered, difficulty, viewDurationMs, onComplete, sound, feedback]);
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] gap-6 p-6">
        ...
        <div className="challenge-zone text-center text-sm text-gray-400 max-w-md leading-relaxed mb-4">
          Read the story carefully. Then answer questions about the details you remember.
          <br /><br />
          <span className="text-pixel-gold font-pixel text-pixel-xs">
            {config.sentenceCount} sentences — {config.viewingTime}s — {config.questionCount} questions
          </span>
        </div>
  ...
  // RECALL (questions)
  if (phase === 'recall' && story && story.questions[currentQIndex]) {
    const q = story.questions[currentQIndex];
    return (
      <div className="p-6">
            ...
            <div className="flex flex-col items-center gap-4">
              <div className="flex items-center gap-3">
                ...
                <input
                  ...
                  onKeyDown={(e) => {
                    if (e.key === 'Enter' && numberValue && !feedback) handleAnswer(numberValue);
                  }}
                  disabled={!!feedback}
                  min={q.min}
                  max={q.max}
